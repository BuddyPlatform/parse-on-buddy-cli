...
function uploadMissing(appID, secret, local, remote, callback) {
  ...
  async.eachLimit(filenames, 4, (filename, eachCallback) => {
    uploadFile(appID, secret, local[filename], filename, eachCallback);
  }, callback);
}
...
function createVersionExecute(appID, secret, version, local, remote, callback) {
  async.parallel({
    sync(callback) {
...
function createVersion(appID, secret, version, callback) {
  async.parallel({
    local(callback) {
    ...
    if (error !== null) {
      ...
      createVersionExecute(appID, secret, version, results.local, results.remote, callback);
    }
  });
}
